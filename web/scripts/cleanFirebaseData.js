/**
 * Firebase Data Cleanup Script (JavaScript version)
 * Run with: node scripts/cleanFirebaseData.js
 */
  ...
  scoreMap.forEach((entries, key) => {
    if (entries.length > 1) {
      ...
      for (let i = 1; i < entries.length; i++) {
        ...
        duplicateCount++;
        const date = new Date((entries[i].createdAt?.seconds || 0) * 1000).toLocaleString();
        console.log(`  ❌ Marking for deletion: ${entries[i].id} (${date})`);
      }
      const keepDate = new Date((entries[0].createdAt?.seconds || 0) * 1000).toLocaleString();
      console.log(`  ✅ Keeping: ${entries[0].id} (${keepDate})`);
    }
  });
...
async function cleanOldRooms() {
  ...
  for (const [roomId, room] of Object.entries(rooms)) {
    ...
    if (age > ONE_DAY) {
      ...
      console.log(`  🗑️  Deleting old room: ${roomId} (${days} days old)`);
...
async function cleanEmptyUserStats() {
  ...
  for (const [userId, userStats] of Object.entries(stats)) {
    ...
    if (singlePlayerPoints === 0 && multiplayerPoints === 0 && multiplayerGamesPlayed === 0) {
      const name = userStats.displayName || 'Unknown';
      console.log(`  🗑️  Deleting empty stats for: ${userId} (${name})`);
